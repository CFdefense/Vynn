...
/ Class SignupPayload: Payload to send to backend for creation of a new user
...
export class Signup {
	...
	constructor(
		new_name: string,
		new_email: string,
		new_password_one: string,
		new_password_two: string
	) {
		this.name = new_name;
...
export async function attempt_signup(signup_input: Signup): Promise<boolean> {
	...
	if (signup_input.password_one != signup_input.password_two) {
		...
		signup_payload = new SignupPayload(
			signup_input.name,
			signup_input.email,
			signup_input.password_one
		);
	}
