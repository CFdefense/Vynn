	...
	async function searchUsers(query: string): Promise<void> {
		...
		try {
			...
			searchResults = data.filter(
				(user: { id: number }) => !usersWithAccess.some((existingUser) => existingUser.id === user.id)
			);
		} catch (err) {
...
{#if isOpen}
	<section class="modal-wrapper" role="dialog" aria-modal="true" aria-labelledby="modal-title">
		<button class="modal-overlay" on:click={closeModal} aria-label="Close modal"></button>
		<div class="modal-content" transition:fade role="document">
			<div class="modal-header">
				...
				<button
					...
					on:keydown={(e) => e.key === 'Escape' && closeModal()}>×</button
				>
			</div>
					...
					<div class="users-list">
						{#each usersWithAccess as user}
							<div class="user-item">
								...
								{#if !isCurrentUser(user)}
									<button class="remove-button" on:click={() => handleRemoveUser(user)} aria-label="Remove user">
										×
									</button>
								{:else}
